...
| Every publishing step stays gated on `RELEASING`, and the release refuses to run with `NUGET_USER` unset | A preflight must not publish; an unset user would skip the push yet still create a GitHub release | `release.yml`, step "Refuse to release without NUGET_USER"; `PackagingTests.The_release_refuses_to_run_without_NUGET_USER`, `The_release_workflow_has_a_credential_preflight`, `The_release_workflow_still_has_both_publishing_steps` |
| The release refuses to run from a repository the conventions reject, ungated so the preflight runs it too | A problem the preflight dispatch finds costs nothing; the same problem found by a tag leaves a pushed tag and a failed release to clean up | `release.yml`, step "Refuse to release a repository the conventions reject"; `PackagingTests.The_release_refuses_to_run_from_a_repository_the_conventions_reject` |
