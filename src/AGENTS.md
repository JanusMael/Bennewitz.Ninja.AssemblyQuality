...
| `Id` is unique, is `BNAQ` and four digits, and never changes; `Summary` states the requirement and ends with a full stop | Consumers suppress by id (`IAssemblyRule.Id`); the summary is the README row (`IAssemblyRule.Summary`) | `RulesCatalogTests.EveryRuleId_IsUnique`, `EveryRuleId_CarriesTheFamilyPrefix`, `EveryRule_SummarisesWhatItRequires`, `TheCatalogue_IsNotSilentlyEmpty` |
...
| Every rule has a `### {Id}` section in the README, naming its `BNCQ` counterpart where one exists | Bennewitz.Ninja.CodeQuality links each source-side rule to that anchor | `RulesCatalogTests.EveryRule_HasAReadmeSectionOfItsOwn` |
