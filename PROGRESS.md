...
| _pending_ | feat: `AssemblyRuleResult.Skipped` names what a rule could not examine (references that would not load, types whose signatures name a missing assembly — previously a crash in AQ1001/AQ1002); `NamespaceShadowRule.IncludingInternalTypes()`. fix: AQ1004 reads forwarded types, so a root reachable only through a facade such as `System.Runtime` is compared against — before this, such an assembly's `.System` shadows went unreported |
